...
def make_generator_model():
    ...
    # model = Sequential()
    # # ,kernel_regularizer = regularizers.l2(0.01)
    # model.add(Conv1D(filters=624, kernel_size=2,
    #                  input_shape=(n_sequence, n_features), activation='elu'))
    # model.add(Conv1D(filters=512, kernel_size=2,  activation='elu'
    #                  ))
    # model.add(MaxPooling1D(pool_size=2))
    # model.add(Flatten())
    ...
    # model.add(Dense(g_output_dim, activation='softmax'))
    # min_delta_val = 0.00001
    # lr_cb = ReduceLROnPlateau(monitor='g_loss',
    #                           factor=0.5, min_delta=min_delta_val, patience=10, verbose=1)
    # es_cb = EarlyStopping(monitor='g_Mac_f1-score',
    #                       min_delta=min_delta_val, patience=50, verbose=1, restore_best_weights=True, mode='max')
...
# %%--------------------------生成器指标获取------------------------
    ...
    def train(self, real_x, real_y, yc, opt):
        ...
        plt.xlabel('iterations of batch')
        plt.ylabel('Loss')
        plt.grid(axis='y', ls='--')
        plt.legend()
